...
class ViTClassification(nn.Module):
    """
    基于 ViT 的分类模型，用于股票收益率的三分类任务（标签：0, 1, 2）
    """
    def __init__(
        self,
        image_size=8,
        ...
        dropout=0.1,
        emb_dropout=0.1,
        pool='cls',              # 池化方式：'cls' 或 'mean'
        num_classes=3            # 分类类别数（0, 1, 2）
    ):
        super().__init__()
        
        # 创建 ViT 主干网络
        # 将 num_classes 设置为分类类别数
        self.vit = ViT(
            image_size=image_size,      # 特征图尺寸
            patch_size=patch_size,      # Patch 大小
            num_classes=num_classes,    # 输出维度=3（三分类任务）
            dim=dim,                    # Transformer 嵌入维度
            depth=depth,                # Transformer 层数
            heads=heads,                # 注意力头数
            mlp_dim=mlp_dim,           # MLP 隐藏层维度
            pool=pool,                  # 池化方式
            channels=channels,          # 输入通道数（7个股票特征）
